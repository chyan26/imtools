<html>
<head><title>wircam processing</title>
</head>
<body>
...
<tr><td>runid</td><td><input type=text size=6 name=runid></td></tr>

<tr><td>scripts</td><td><input type=checkbox name=processinfo>processinfo <input type=checkbox name=skyscript>skyscript</td></tr>

<tr><td>RAWPATH</td><td><input name=rawpath type=text size=40></tr>
<tr><td>REDPATH</td><td><input name=redpath type=text size=40></tr>
<tr><td>CALIBPATH</td><td><input name=calibpath type=text size=40></tr>

<tr><td>DP</td><td><input name=dppatern type=text size=10 value='DP'></tr>

<tr><td colspan=2 align=center><input type=submit name=generate value=generate></td></tr>
...
$runid = $_GET['runid'];
$processinfo = $_GET['processinfo'];
$skyscript = $_GET['skyscript'];
...
if ($runid!="" && $processinfo=='on') {
/** Inintialize the script page. */
printf("#! /bin/bash  <br>");
printf("#### THIS IS THE PROCESSINFO SCRIPT<br>");
printf("<br>");
...
for ($i=0;$i<sizeof($filename);$i++){
        ...
        $target=array("odometer"=>$odometer[$i],
                "date"=>$date[$i],"ra"=>$ra[$i],"dec"=>$dec[$i]);

        $skylist=getSkyFrames($target,$odometer,$date,$ra,$dec);
...
if ($runid!="" && $skyscript=='on') {
/** Inintialize the script page. */
printf("#! /bin/bash  <br>");
printf("#### THIS IS THE SKYSCRIPT SCRIPT<br>");
printf("<br>");
